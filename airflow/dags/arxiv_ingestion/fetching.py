...
def fetch_metadata(**context):
    ...
    storage - so this is the only step retried when arXiv throttles a request (see
    `ArxivClient`'s docstring for arXiv's retry-worthy quirks and why that can take
    a while). `ArxivAPIRateLimitError`/`ArxivAPIServerError` are left to propagate
    for Airflow's own task-retry policy; `ArxivAPIClientError` (a permanent 4xx,
    e.g. a malformed query) is converted to `AirflowFailException` here so it fails
    the task immediately instead of burning through Airflow's retries too.
    """
